...
from scrappers.items import AntItem

class AntSpider(Spider):
    name = "ant"
    allowed_domains = ["consultaweb.ant.gob.ec"]
    start_urls = ["https://consultaweb.ant.gob.ec/PortalWEB/paginas/clientes/clp_grid_citaciones.jsp?ps_tipo_identificacion=CED&ps_identificacion={}&ps_placa="]

    custom_settings = {
        'ITEM_PIPELINES' : {
            "scrappers.pipelines.DPPAntPipeline": 300,
    ...
    def start_requests(self):
        yield Request(self.start_urls[0].format(self.search_id), callback=self.parse, meta={'id': self.search_id})
    ...
    def parse(self, response, **kwargs):
        if response.xpath('//td[contains(text(),"LICENCIA TIPO")]/text()'):
            for record in response.xpath('//td[contains(text(),"LICENCIA TIPO")]/text()'):
                ...
                ant_item["cedula"] = response.xpath('//td[contains(text(),"CED")]/text()').get('').split('-')[-1].strip()
                ant_item["license_type"] = record.root.split('/')[0].split(':')[-1].split('&')[0].strip()
                ant_item["expedition_date"] =  record.root.split('VALIDEZ:')[-1].split(' - ')[0]
                ant_item["expiration_date"] = record.root.split('VALIDEZ:')[-1].split(' - ')[-1]
                ant_item["points"] = response.xpath('//td[div[contains(text(),"Puntos")]]/following-sibling::td[1]/text()').get('')
                ant_item["total"] = ""
                ...
                yield Request(
                    ...
                    meta={'item': ant_item}, dont_filter=True)
        else:
            raise CloseSpider("not found")

    def total_value(self, response):
        item = response.meta['item']
        item['total'] = response.xpath('//td[font[contains(text(),"TOTAL:")]]/following-sibling::td[1]/font/text()').get('').strip()
        yield item